...
from referance.HAMNet.ham.src.ham.util.config import ConfigBase

from referance.HAMNet.ham.src.ham.env.env.iface import EnvIface
from referance.HAMNet.ham.src.ham.env.common import set_vulkan_device
from referance.HAMNet.ham.src.ham.env.scene.base import SceneBase
from referance.HAMNet.ham.src.ham.env.robot.base import RobotBase
from referance.HAMNet.ham.src.ham.env.task.base import TaskBase
from referance.HAMNet.ham.src.ham.env.common import (
...
class TrackObjectAssets:
    ...
    def create_box(self, sim, width: float, height: float, depth: float,
                   options=None):
    ...
    def load_urdf(self, sim, rootpath: str, filename: str,
                  options=None):
...
class EnvBase(EnvIface):
    ...
    def __init__(self,
        ...
        # NOTE(ycho): ad-hoc modification to
    ...
    def num_rew(self) -> int:
        return self.task.num_rew

    @property
    def num_type(self) -> int:
        return self.task.num_type
    ...
    def setup(self):
        ...
        # FIXME: Need better tune
        self.gym.set_light_parameters(
            self.sim, 0, gymapi.Vec3(0.5, 0.5, 0.5),
            gymapi.Vec3(0.5, 0.5, 0.5),
            gymapi.Vec3(-0.57735, -0.57735, -0.57735))

        # Setup robot, scene, task
        self.robot.setup(self)
        self.scene.setup(self)
        self.task.setup(self)

        # Create viewer & configure camera
        if cfg.use_viewer:
    ...
    def compute_feedback(self,
                         ...
                         action: th.Tensor):
        """
        NOTE(ycho): technically, the complete
        specification of the feedback/reward function
        should be R(s,a,s') = (prev_state, action, next_state)
        """
    ...
    def reset_indexed(self,
        ...
        # [1] commit the `root_state` tensors.
        if len(id_robot) <= 0:
            set_ids = id_scene
        else:
            # NOTE(ycho): is the sort needed?
        ...
        # [2] commit the `dof_state` tensors (usually robot joints)
        ...
        # NOTE(ycho): len(id_robot) cannot be <=0.
    ...
    def step(self, actions):
        ...
        if (hasattr(self, 'viewer')
            ...
            for evt in self.gym.query_viewer_action_events(self.viewer):
                ...
                if evt.value <= 0:  # ???
                    continue
                [cb() for cb in self._event_cb[evt.action]]
    ...
    def pre_physics_step(self, actions):
        ...
        * Apply actions from policy.
        ...
        with nvtx.annotate("apply_actions"):
            # NOTE(ycho):
            # apply_actions() in base classes should _not_ apply
            # any actions for which `done`=True,
            # since those environments will be reset right after,
            # and may still be influenced by these actions.
    ...
    def physics_step(self):
        ...
        * Run physics simulation.
        ...
        # Step physics N>=1 times.
        if cfg.ig_ctrl:
            # Using isaac gym's internal controller.
            # if using ig_ctrl:
            # * call step_controller() _once_ at the beginning.
            # * no intermediate refresh_tensors() necessary
            # * we also skip the internal self.render() since
            #   we prefer not to have to call refresh_tensors().
            self.render()
            self.robot.step_controller(self.gym, self.sim, self)
            for i in range(cfg.action_period):
                with nvtx.annotate("gym.simulate"):
                    if self._step_phys:
                        self.gym.simulate(self.sim)
        else:
            # Using "external" controller requires tensor update.
    ...
    def render(self):
        ...
        if self.gym.query_viewer_has_closed(self.viewer):
            ...
            return
        # TODO(ycho): process keyboard events here.
        if self.device != 'cpu':
            self.gym.fetch_results(self.sim, True)
    ...
    def create_env(self, env_id: int) -> Tuple[List, Dict, Dict]:
        ...
        # Determine physics group aggregation flags
        ...
        # Validate that max-counts are not exceeded
        if use_agg_all:
            shape_count = 0
    ...
    def acquire_tensors(self):
        ...
        # NOTE(ycho): 13 = (pos(3)+orn(4)+lin_vel(3)+ang_vel(3)
        tensors = {}
        ...
        if cfg.measure_joint_torque:
            descriptors['joint_torque'] = gym.acquire_dof_force_tensor(sim)
            tensors['joint_torque'] = gymtorch.wrap_tensor(
                descriptors['joint_torque']).view(
                cfg.num_env, -1)
