...
from ham.env.scene.bench_object_set import BenchObjectSet
...
from ham.env.episode.pre_sample_pose import UsePresampledPose, GetObjectOrn
...
def get_eps_generator(is_mesh_env: bool = False,
                      sample_curobo: bool = True,
                      use_presampled_pose: bool = False,
                      ):
    if is_mesh_env:
        gen_eps = Compose([
            TableDim(),
            ...
            ((InitObjectOrn(),
              InitObjectPose(),
              GoalObjectPose()) 
            if not use_presampled_pose else
            (GetObjectOrn(),UsePresampledPose())),
            RobotDof(),
            (CollisionFree()
             if sample_curobo
             else SelPose())
...
class TableTopWithObjectScene(TableTopScene):
    ...
    class Config(TableTopScene.Config):
        ...
        bench: BenchObjectSet.Config = BenchObjectSet.Config()
        ...
        use_pre_sampled_pose: bool = False

        lower_table_pos_bound: Tuple[float, float] = (0.0, -0.15)
        upper_table_pos_bound: Tuple[float, float] = (0.1, 0.15)
    ...
    def __init__(self, cfg: Config):
        ...
        # Episode generator...
        self.gen_eps = get_eps_generator(cfg.mesh_env,
                                         cfg.sample_curobo,
                                         cfg.use_pre_sampled_pose)
        ...
        table_dim_lower_bound = table_base_dims * 0.5 if cfg.use_table_dim_dr else table_base_dims
        table_dim_bound = np.stack([table_dim_lower_bound,
                                    table_base_dims])
        table_pos_bound = (
            (*cfg.lower_table_pos_bound, cfg.min_table_height_offset),
            (*cfg.upper_table_pos_bound, cfg.max_table_height_offset)
        )
