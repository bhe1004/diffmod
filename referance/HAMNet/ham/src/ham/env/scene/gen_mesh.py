...
class MeshGen:
    ...
    def geom(self, dim_table: th.Tensor):
        B = dim_table.shape[0]
        # NOTE just a placeholder to meet requirements
        return th.zeros(B,1,3, dtype=th.float,
    ...
    def pose(self,
        ...
        pose = th.zeros(B, 7, dtype=th.float,
                        device=dim_table.device)
        pose[..., 2] = z_table
        # quat
        pose[..., -1] = 1
        c = dict(has_ceil=(self.cfg.has_ceil*
                           th.ones(B, dtype=th.bool,
                                   device=dim_table.device)),
                height=th.ones(B, dtype=th.float,
                               device=dim_table.device))
        return dict(pose=pose[..., None, :],
                    case=c)
    ...
    def cloud(self, workspace, dim_table, meta, num_samples: int,
        ...
        # Workaround for support cloud from meshenv
        mask.fill_(1)
    ...
    def __sample(self,
                 ...
                 eps: float,
                 pose_meta=None):
        mask = ((ws_lo[..., None, :2] - eps <= points[..., None, :, :2]).all(dim=-1)
                & (points[..., None, :, :2] <= ws_hi[..., None, :2] + eps).all(dim=-1))
        ...
        if pose_meta is not None:
            sampled_points[..., 2] += pose_meta['pose'][..., None, 2]
    ...
    def place(self,
              ...
              **kwds):
        ws_lo, ws_hi = workspace.unbind(dim=-2)
        ws_lo = ws_lo[..., None, :].expand(-1, obj_radius.shape[-1], -1).clone()
        ws_hi = ws_hi[..., None, :].expand(-1, obj_radius.shape[-1], -1).clone()

        if init:
            margin = obj_radius[..., None]
            ws_lo[..., :2].clamp_min_(-0.5 * dim_table[..., None, :2] + margin)
            ws_hi[..., :2].clamp_max_(0.5 * dim_table[..., None, :2] - margin)
