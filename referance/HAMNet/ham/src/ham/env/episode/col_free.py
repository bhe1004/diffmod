...
class CollisionFree(DefaultSpec):
    ...
    def sample_setup(
        ...
        for i in range(num_env):
            ...
            if fuse_pose:
                scene_urdf_file = f'{tmpdir}/table_{i:02d}.urdf'
                scene_urdf = URDF.load(scene_urdf_file,
                             build_scene_graph=False,
                             load_meshes=False)
                for k, v in scene_urdf.link_map.items():
                    for j, c in enumerate(v.collisions):
                        mesh_file = c.geometry.mesh.filename
                        pose = c.origin[:3,-1].tolist() + [1, 0, 0, 0]
                        mesh_data[F'table_{i:03d}_{j:02d}'] = {
                            'pose': pose,
                            'file_path': mesh_file
                        }
                world_datum = {
    ...
    def sample_reset(self,
        ...
        if fuse_pose:
            col_world._mesh_tensor_list[1][reset_ids, 1:, 0:7] = (
                plate_pose
            )
            # Currently objec is 0 and env is 1
            out = _finalize_curobo_pose(robot_pos,
                                        col_world._mesh_tensor_list[1][..., 1:, :],
                                        reset_ids)
        else:
        ...
        if fuse_pose:
            # Currently objec is 0 and env is 1
            col_world._mesh_tensor_list[2][..., 1:, :].fill_(0)
        else:
        ...
        if near_obj:
            ...
            if fuse_pose:   
                _finalize_curobo_pose(robot_pos,
                                    col_world._mesh_tensor_list[1][..., :1, 0:7],
                                    reset_ids)
            else:
                _finalize_curobo_pose(robot_pos,
