...
from referance.HAMNet.ham.src.ham.util.torch_util import merge_shapes
...
def _map_struct(
    ...
    # NOTE(ycho): Mapping should _always_ come before
    # Iterable, since Mapping often also an iterable.
    # for instance, dict() is also an iterable.
    # map_mapping()
    # FIXME(ycho): nn.ModuleDict specialization is required
...
def print_struct(x):
    print(map_tensor(x, lambda src, _ : dict(s=src.shape, t=src.dtype, d=src.device)))
