...
from referance.HAMNet.ham.src.ham.models.common import (
    merge_shapes,
    MLP,
    ortho_init,
...
class PiNet(nn.Module):
    ...
    def __init__(self, cfg: Config):
        ...
        if cfg.train_log_std:
            self.register_parameter('log_std',
                                    nn.Parameter(th.full(ls_dim,
                                                         cfg.log_std_init),
                                                 requires_grad=True))
        else:
            self.register_buffer('log_std', th.full(ls_dim,
                                                    cfg.log_std_init),
                                 persistent=True)
        if cfg.ortho_init:
...
class MultiCategoricalPiNet(nn.Module):
    """ Multi-Categorical Policy """
    @dataclass
    class Config(ConfigBase):
        dim_feat: int = -1
        dim_hidden: S = ()
    ...
    def __init__(self, cfg: Config):
        super().__init__()
        self.cfg = cfg
        dims = merge_shapes(cfg.dim_feat, cfg.dim_hidden, np.prod(cfg.dim_act))
        act_cls = get_activation_function(cfg.act_cls)
        self.logits = MLP(dims, act_cls, False, cfg.use_bn,
                          use_ln=cfg.use_ln,
                          bias=cfg.bias,
...
class NoOpPiNet(nn.Module):
    @dataclass
    class Config(ConfigBase):
        dim_act: Tuple[int, ...] = ()
        override_log_std: bool = True
        split_log_std: bool = False
        train_log_std: bool = False
        log_std_init: float = -0.4
    ...
    def __init__(self, cfg: Config):
        ...
        self._hack_env_type = None
        if cfg.override_log_std:
            if cfg.split_log_std:
                ls_shape = merge_shapes(4, cfg.dim_act)
            else:
                ls_shape = cfg.dim_act

            # hardcoded -0.4 start
            ls = th.zeros(ls_shape, dtype=th.float) + cfg.log_std_init

            if cfg.train_log_std:
                self.register_parameter(
                    'log_std', nn.Parameter(
                        ls, requires_grad=True))
            else:
                self.register_buffer('log_std', ls, persistent=True)
    ...
    def forward(self, x: th.Tensor,
        ...
        if cfg.override_log_std:
            ...
            if cfg.split_log_std:
                log_std = log_std[self._hack_env_type]
                ls = log_std
            else:
                # FIXME(ycho): assumes diagonal gaussian
                ls = log_std.expand_as(mu)
        else:
            ls = x[..., a:a + a]
        return mu, ls
...
class MLPFwdBwdDynLossNet(nn.Module):
    ...
    def forward(self, states: th.Tensor, actions: th.Tensor):
        ...
        s1 = states[:, 1:]
        a = actions[:, :-1]
        a = a.reshape(a.shape[:-len(cfg.dim_act)], -1)
