...
from referance.HAMNet.ham.src.ham.models.rl.net.base import (
...
class MLPFuser(nn.Module, FuserBase):
    ...
    def __init__(self,
        ...
        # FIXME(ycho): cannot handle nested mappings
        if isinstance(input_dims, Mapping):
            if cfg.keys is not None:
                # WARN(ycho): we do _not_ sort the input keys
...
class MLPAggNet(nn.Module, AggregatorBase):
    ...
    def __init__(self, cfg: Config):
        ...
        self.num_act = np.prod(dim_act)

        dim_in: int = (dim_obs[0] + self.num_act)
    ...
    def forward(self,
        ...
        if (a is not None) and (self.num_act > 0):
            if not th.is_floating_point(a):
                a = F.one_hot(a.long(), cfg.dim_act[-1])
