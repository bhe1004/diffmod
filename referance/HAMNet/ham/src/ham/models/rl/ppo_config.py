...
def domain_config_from_env(env: EnvIface, **kwds) -> DomainConfig:
    ...
    discrete: bool = isinstance(env.action_space,
                                (spaces.Discrete, spaces.MultiDiscrete))
    multi_discrete: bool = isinstance(env.action_space,
                                      spaces.MultiDiscrete)
    ...
    if isinstance(env.action_space, spaces.Discrete):
        ...
        # Currently FORCED to be like:
        # N x B
        num_act = [len(env.action_space.nvec),
                   env.action_space.nvec[0]]
    elif isinstance(env.action_space, spaces.Box):
        num_act = env.action_space.shape

    # NOTE(ycho): remaining items that are supplied from kwds:
