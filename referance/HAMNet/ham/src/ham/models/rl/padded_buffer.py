...
import torch as th
from referance.HAMNet.ham.src.ham.models.common import merge_shapes, map_struct
...
class DictBuffer:
    ...
    def add_field_from_tensor(self, key: str, value: th.Tensor):
        # TODO(ycho): rename from `_tensor` -> `_sample`?
        key_map = KeyMap(key)
        ...
        with th.no_grad():
            ...
            if p2 >= self.size:
                ...
                if n >= self.size:
                    ...
                    # NOTE(ycho): this specialization _might_
...
class PaddedBuffer:
    ...
    def initialize(self, obs: T, act_dim: int,
        ...
        pre_init_frame = {
            ...
            # TODO(ycho): `distribution` or `dist-params` instead,
        ...
        init_frame = {
            ...
            'actn': th.zeros(act_shape, dtype=act_dtype, device=self.device),
            'rewd': th.zeros((N,), dtype=th.float, device=self.device),
            # TODO(ycho): `done` not guaranteed to be false.
    ...
    def clear(self, keep_last: bool = True):
        ...
        if keep_last:
            assert (self.size >= 2)
            for k in self.KEYS:
                # FIXME(ycho): ONLY works if running clear() right after full
