...
Prerequisites (inside the docker container): ensure benchmark assets are downloaded as described in `ASSET.md`.

```bash
# From repo root
bash download.sh --bench
```

Run benchmark evaluation across all domains:
...
Results will be written under `/tmp/docker/result/ours/<domain>`.
...
The plot will be saved to `/tmp/docker/bench_result.pdf` and a parsed summary to `/tmp/bench_result.pkl`.