...
class CountCategoricalSuccess(WrapperEnv):
    def __init__(self, env: EnvIface,
                 category: str = 'obj',
                 save_dir: str = '/tmp/docker/result/'
                 ):
        super().__init__(env)
        self._save_dir: Path = ensure_directory(save_dir)
    ...
    def _export(self):
        index = self.export_count
        with open(str(self._save_dir / F'episode-{index:04d}.pkl'), "wb") as fp:
            pickle.dump(self.episode_data, fp)
        self.export_count += 1
        self.episode_data = []
    ...
    def step(self, actions: th.Tensor):
        ...
        if len(env_reset_indices) > 0:
            for idx in env_reset_indices:
                ...
                if 'success' in info:
                    episode_data['success'] = info['success'][ii].item()
                if scene.data['obj_name'] is not None:
                    episode_data['object_name'] = str(
                        scene.data['obj_name'][ii])
                if scene.data['obj_radius'] is not None:
                    episode_data['object_radius'] = (
                        scene.data['obj_radius'][ii].item()
                    )
                
                self.episode_data.append(episode_data)

                if len(self.episode_data) > 1024:
                    self._export()
