...
**Download Benchmark Assets/Episodes**

To download the assets required for benchmarking (real scenes, GSO models, DGN bench subset, domain configs, and pre-sampled evaluation episodes), run inside the docker container:
```bash
bash download.sh --bench
```

After completion, the directory structure inside the container should include:
